...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      {/* Scanline effect */}
      <div className="scanline pointer-events-none fixed top-0 left-0 w-full h-full"></div>
      
      {/* HUD Header */}
      <header className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-eva-purple/30 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-eva-purple flex items-center justify-center rounded-sm transform rotate-45">
            <Shield className="w-6 h-6 text-black transform -rotate-45" />
          </div>
          <div>
            <h1 className="text-2xl font-bold tracking-tighter mono leading-none">LYES</h1>
            <p className="text-[10px] text-eva-purple uppercase tracking-widest mono">Nerv Technical Division</p>
          </div>
        </div>

        <nav className="hidden md:flex space-x-8 uppercase mono text-sm font-medium tracking-widest">
      ...
      <footer className="bg-black border-t border-eva-purple/20 p-8 mt-20 relative overflow-hidden">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-center md:text-left">
            <p className="mono text-xs text-gray-500 uppercase tracking-widest">Copyright © 2024 Lyes Technical Solutions</p>
            <p className="mono text-[10px] text-eva-purple mt-1 tracking-widest">AUTHORIZED PERSONNEL ONLY - TOP SECRET</p>
          </div>
        ...
        <div className="absolute bottom-0 right-0 p-4 opacity-20 pointer-events-none">
          <div className="border-r-4 border-b-4 border-eva-orange w-12 h-12"></div>
        </div>
      </footer>
    </div>
  );
